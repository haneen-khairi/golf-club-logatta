...
?>">
	...
	if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']=='image')) {
		...
		?><a href="<?php echo esc_url($link); ?>"
			...
			if (!empty($svg)) {
				...
				?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="'.esc_attr__('icon', 'triompher').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			}
		?></a><?php
	} else {
	...
	<div class="sc_services_item_info">
		...
		<?php if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) { ?>
			...
			<div class="sc_services_item_button sc_item_button "><a href="<?php echo esc_url($link); ?> " class="sc_button sc_button_default sc_button_size_normal sc_button_icon_left sc_button_hover_slide_left"><?php esc_html_e('Learn more', 'triompher'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
